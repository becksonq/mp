...
/**
 * Это библиотека simplehtmldom для парсинга html: http://simplehtmldom.sourceforge.net/
 * Портированная для Yii2 Адаменко Денисом [drimd@ukr.net]
 * 
 * Изменения: добавил комментарии с описанием;
 * добавил метод парсинга с помощью cURL, вместо file_get_contents()
 * методы называются: curl_request() и file_curl_get_html(); именно последний метод позволяет очень легко парсить страницы
...
namespace darkdrim\simplehtmldom;
...
class SimpleHTMLDom extends \yii\helpers\Inflector
{
	...
	public static function file_get_html($url, $use_include_path = false, $context = null, $offset = -1, $maxLen = -1, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT) 
	{
		...
		if(SimpleHTMLDom::get_http_response_code($url) != "404")
		{
		...
		// Это метод парсинга строки, в которой присутствует html
	...
	public static function curl_request($url, $post = 0, $timeout = 30, $path_to_cookie = '@web/cookie/cookie.txt')
	{
		$ch = curl_init();
		...
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $timeout);// Таймаут
		...
		// Парсинг страницы, используя cURL()
	public static function file_curl_get_html($url, $post = 0, $timeout = 30, $path_to_cookie = '@web/cookie/cookie.txt',
	...
	{
		return SimpleHTMLDom::str_get_html(SimpleHTMLDom::curl_request($url, $post, $timeout, $path_to_cookie), $lowercase, $forceTagsClosed, $target_charset,		$stripRN, $defaultBRText, $defaultSpanText);
	}
}
...
if (is_object($debug_object)) {
	$debug_object->debugLog(3, "source charset: " . $sourceCharset . " target charaset: " . $targetCharset);
}
...
class simple_html_dom {
	...
	for ($i = $count - 1; $i > -1; --$i) {
		...
		if (is_object($debug_object)) {
			$debug_object->debugLog(2, 'key is: ' . $key);
		}
		$idx = ($remove_tag) ? 0 : 1;
